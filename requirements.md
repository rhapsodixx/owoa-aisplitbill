...
- **Security:**
  ...
  - **Brute-Force Protection (MANDATORY):**
    - **Tracking:** Failures tracked by `uuid` + `client_identifier`.
    - **Client Identifier Priority:**
      1. Authenticated Session ID (if available).
      2. SHA-256 Hash(IP Address + User-Agent). **Do not store raw IP.**
    - **Policy:**
      - **Max Attempts:** 5 failed attempts within a 15-minute window.
      - **Lockout:** After 5 failures, block access for 15 minutes.
      - **Reset:** Successful entry resets the counter immediately.
    - **Response:**
      - On Lockout: Return `429 Too Many Requests` with `retry-after` header/body.
      - UI MUST show a friendly "Too many attempts" message with the remaining wait time.
    - **Privacy:**
      - Must NOT leak existing UUIDs via different error messages (maintain current 404 behaviors).
      - NEVER log passcodes in plaintext.
...
### Rate Limiting Storage (New)

- **Table:** `passcode_attempts`
- **Purpose:** Track failed web-access attempts to private results.
- **Fields:**
  - `result_id`: UUID (FK to `results.id` ON DELETE CASCADE)
  - `client_key`: TEXT (Hash of IP + User-Agent, or Session ID)
...
> [!CAUTION] > `payment_instruction` is **NOT** AI input and must NEVER be processed by the LLM.
...
### Security & Brute-Force Tests (Playwright + Gherkin)

- **Scenario: Progressive Lockout**

  - Given a private result exists
  - When a user enters the wrong passcode 5 times within 15 minutes
  - Then the user sees a "Too many attempts" lockout message
  - And the user cannot submit another passcode until the lockout expires

- **Scenario: Correct Passcode Resets Counter**

  - Given a user has 4 failed attempts
  - When the user enters the correct passcode
  - Then the result is displayed
  - And the failed attempt counter is reset to 0

- **Scenario: Lockout Persists Even With Correct Passcode**

  - Given a user is locked out
  - When the user tries to enter the _correct_ passcode
  - Then the access is still denied with a lockout message

- **Scenario: Client Fingerprint Isolation**
  - Given User A is locked out
